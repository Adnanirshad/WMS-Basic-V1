...
ESSApp.controller('LvForwardCtrl', function ($scope, $http, filterFilter) {


    // init function
    $scope.initFunction = function () {
        // get Locations
        $http.get('/LeaveForward/PendingLeavesList/').success(function (res) {
            $scope.AllLeaves = res;
    ...
    $scope.approveAll = function () {
        $http.get('/LeaveForward/ApproveAll/').success(function (res) {
            if (res === 'success') {
    ...
    $scope.approveSelected = function () {
        ...
        $http.post("/LeaveForward", JSON.stringify(selectedLeaves)).success(function (data) {
    ...
    $scope.approveLeave = function (leave) {
        $http.get("/LeaveForward/ApproveLeave?LvID=" + leave.LvID).success(function (result) {
            if (result === 'success') {
    ...
    $scope.revokeLeave = function (leave) {
        $http.get("/LeaveForward/RevokeLeave?LvID=" + leave.LvID).success(function (result) {
            if (result === 'success')
                removeLeaveFromList(leave);
        })
    }
